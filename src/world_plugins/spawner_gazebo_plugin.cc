...
namespace gazebo
{
  ...
  class SpawnerPlugin : public WorldPlugin
  {
  ...
  void SpawnerPlugin::Load(physics::WorldPtr _world, sdf::ElementPtr _sdf) {
    ...
    gz_uav_sub_ = gz_node_->Subscribe("~/pose/info",&SpawnerPlugin::callbackGazeboUavPose, this, 1);
  ...
  void SpawnerPlugin::callbackGazeboUavPose(const GazeboPosesStampedConstPtr& msg) {
exists = false;
...
if (!exists)
  ...
  GZ_REGISTER_WORLD_PLUGIN(SpawnerPlugin)
}
